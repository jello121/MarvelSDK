#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: PyAbility_101651

#include "Basic.hpp"

#include "Hero_1016_classes.hpp"
#include "Marvel_classes.hpp"


namespace SDK
{

// PythonClass PyAbility_101651.PyUIController_101651
// 0x0000 (0x0CD0 - 0x0CD0)
class UPyUIController_101651 final : public UUIC_Ability
{
public:
	void OnInit();
	void OnDestruct();

public:
	static class UClass* StaticClass()
	{
		return StaticClassImpl<"PyUIController_101651">();
	}
	static class UPyUIController_101651* GetDefaultObj()
	{
		return GetDefaultObjImpl<UPyUIController_101651>();
	}
};
static_assert(alignof(UPyUIController_101651) == 0x000008, "Wrong alignment on UPyUIController_101651");
static_assert(sizeof(UPyUIController_101651) == 0x000CD0, "Wrong size on UPyUIController_101651");

// PythonClass PyAbility_101651.PySummoned_10165101
// 0x0010 (0x13A0 - 0x1390)
class APySummoned_10165101 : public ASummoned_10165101
{
public:
	FMulticastInlineDelegateProperty_             DelegateOnHidePhantomWhenTeleport;                 // 0x1390(0x0010)(ZeroConstructor, InstancedReference, BlueprintAssignable, BlueprintCallable, NativeAccessSpecifierPublic)

public:
	void BindMouseLeftForPhantom(bool bEnable);
	void OnInstigatorAbilityTruncated(const struct FGameplayTagContainer& Tags_0);
	void DelegateOnHidePhantomWhenTeleport__DelegateSignature(bool IsHidden);

public:
	static class UClass* StaticClass()
	{
		return StaticClassImpl<"PySummoned_10165101">();
	}
	static class APySummoned_10165101* GetDefaultObj()
	{
		return GetDefaultObjImpl<APySummoned_10165101>();
	}
};
static_assert(alignof(APySummoned_10165101) == 0x000010, "Wrong alignment on APySummoned_10165101");
static_assert(sizeof(APySummoned_10165101) == 0x0013A0, "Wrong size on APySummoned_10165101");
static_assert(offsetof(APySummoned_10165101, DelegateOnHidePhantomWhenTeleport) == 0x001390, "Member 'APySummoned_10165101::DelegateOnHidePhantomWhenTeleport' has a wrong offset!");

// PythonClass PyAbility_101651.PyAbility_101651
// 0x0008 (0x2AF8 - 0x2AF0)
class UPyAbility_101651 : public UAbility_101651
{
public:
	class ASummonedCharacterBase*                 PhantomCharacter;                                  // 0x2AF0(0x0008)(BlueprintVisible, ZeroConstructor, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

public:
	void BeginPlay();
	void K2_ActivateAbility();
	void K2_OnEndAbility(bool bWasCancelled);
	void NativeOnMontageCompleted(const class FString& Tag);
	void NativeOnMontageInterrupted(const class FString& Tag);
	void NativeOnMontageEvent(const class FString& Tag);
	void SpawnPhantom(const struct FTransform& Transform, class UMarvelGameplayAbility* InAbility);
	void InitPhantom(class AActor* Source);
	void OnConfirmEvent(const struct FGameplayAbilityTargetDataHandle& Data);
	void OnCancelEvent(const struct FGameplayAbilityTargetDataHandle& Data);

public:
	static class UClass* StaticClass()
	{
		return StaticClassImpl<"PyAbility_101651">();
	}
	static class UPyAbility_101651* GetDefaultObj()
	{
		return GetDefaultObjImpl<UPyAbility_101651>();
	}
};
static_assert(alignof(UPyAbility_101651) == 0x000008, "Wrong alignment on UPyAbility_101651");
static_assert(sizeof(UPyAbility_101651) == 0x002AF8, "Wrong size on UPyAbility_101651");
static_assert(offsetof(UPyAbility_101651, PhantomCharacter) == 0x002AF0, "Member 'UPyAbility_101651::PhantomCharacter' has a wrong offset!");

// PythonClass PyAbility_101651.PyCue_Summoner_Loop_10165101
// 0x0008 (0x10F0 - 0x10E8)
class APyCue_Summoner_Loop_10165101 final : public ACue_Summoner_Loop_10165101
{
public:
	class UNiagaraSystem*                         CollabrateReleaseFX;                               // 0x10E8(0x0008)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnInstance, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

public:
	bool OnActive(class AActor* MyTarget, const struct FGameplayCueParameters& Parameters);
	bool WhileActive(class AActor* MyTarget, const struct FGameplayCueParameters& Parameters);
	void WhileActiveFX(class AActor* MyTarget, const struct FGameplayCueParameters& Parameters);
	void OnExecuteFX(class AActor* MyTarget, const struct FGameplayCueParameters& Parameters);
	void OnExecuteAudio(class AActor* MyTarget, const struct FGameplayCueParameters& Parameters);
	bool OnRemove(class AActor* MyTarget, const struct FGameplayCueParameters& Parameters);
	void TryStartCountDown();
	float GetDuration();
	float GetRemainTime();
	void StartCountDown(float RemainTime, float Duration);
	bool IsNeedPlayEffect(class AActor* InActor);
	void SetCheckHp(bool ShouldCheck);
	void LoopCheckNeedShowHP();
	void SetIndicatorHidden(bool IsHidden);

public:
	static class UClass* StaticClass()
	{
		return StaticClassImpl<"PyCue_Summoner_Loop_10165101">();
	}
	static class APyCue_Summoner_Loop_10165101* GetDefaultObj()
	{
		return GetDefaultObjImpl<APyCue_Summoner_Loop_10165101>();
	}
};
static_assert(alignof(APyCue_Summoner_Loop_10165101) == 0x000008, "Wrong alignment on APyCue_Summoner_Loop_10165101");
static_assert(sizeof(APyCue_Summoner_Loop_10165101) == 0x0010F0, "Wrong size on APyCue_Summoner_Loop_10165101");
static_assert(offsetof(APyCue_Summoner_Loop_10165101, CollabrateReleaseFX) == 0x0010E8, "Member 'APyCue_Summoner_Loop_10165101::CollabrateReleaseFX' has a wrong offset!");

// PythonClass PyAbility_101651.PyCue_TraceActor_10165101
// 0x0000 (0x0F18 - 0x0F18)
class APyCue_TraceActor_10165101 final : public ACue_TraceActor_10165101
{
public:
	bool WhileActive(class AActor* MyTarget, const struct FGameplayCueParameters& Parameters);
	bool OnRemove(class AActor* MyTarget, const struct FGameplayCueParameters& Parameters);
	void StartEffect();
	void OnFloatChange(class FName TrackName, class FName PropertyName, float OutValue);
	void StopEffect();

public:
	static class UClass* StaticClass()
	{
		return StaticClassImpl<"PyCue_TraceActor_10165101">();
	}
	static class APyCue_TraceActor_10165101* GetDefaultObj()
	{
		return GetDefaultObjImpl<APyCue_TraceActor_10165101>();
	}
};
static_assert(alignof(APyCue_TraceActor_10165101) == 0x000008, "Wrong alignment on APyCue_TraceActor_10165101");
static_assert(sizeof(APyCue_TraceActor_10165101) == 0x000F18, "Wrong size on APyCue_TraceActor_10165101");

// PythonClass PyAbility_101651.PyLokiSpaceActor
// 0x0000 (0x0610 - 0x0610)
class APyLokiSpaceActor final : public ALokiSpaceActor
{
public:
	void ReceiveBeginPlay();

public:
	static class UClass* StaticClass()
	{
		return StaticClassImpl<"PyLokiSpaceActor">();
	}
	static class APyLokiSpaceActor* GetDefaultObj()
	{
		return GetDefaultObjImpl<APyLokiSpaceActor>();
	}
};
static_assert(alignof(APyLokiSpaceActor) == 0x000008, "Wrong alignment on APyLokiSpaceActor");
static_assert(sizeof(APyLokiSpaceActor) == 0x000610, "Wrong size on APyLokiSpaceActor");

}

