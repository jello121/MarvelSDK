#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: PyAbility_101641

#include "Basic.hpp"

#include "Marvel_classes.hpp"
#include "GameplayTags_structs.hpp"


namespace SDK
{

// PythonClass PyAbility_101641.PyConfig_101641
// 0x0018 (0x00B0 - 0x0098)
class UPyConfig_101641 final : public UMarvelAbilityConfig
{
public:
	int32                                         StealthBuffID;                                     // 0x0098(0x0004)(Edit, BlueprintVisible, BlueprintReadOnly, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
	int32                                         PhantomBuffID;                                     // 0x009C(0x0004)(Edit, BlueprintVisible, BlueprintReadOnly, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
	struct FGameplayTag                           StopCameraInterpTag;                               // 0x00A0(0x000C)(Edit, BlueprintVisible, BlueprintReadOnly, DisableEditOnInstance, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

public:
	static class UClass* StaticClass()
	{
		return StaticClassImpl<"PyConfig_101641">();
	}
	static class UPyConfig_101641* GetDefaultObj()
	{
		return GetDefaultObjImpl<UPyConfig_101641>();
	}
};
static_assert(alignof(UPyConfig_101641) == 0x000008, "Wrong alignment on UPyConfig_101641");
static_assert(sizeof(UPyConfig_101641) == 0x0000B0, "Wrong size on UPyConfig_101641");
static_assert(offsetof(UPyConfig_101641, StealthBuffID) == 0x000098, "Member 'UPyConfig_101641::StealthBuffID' has a wrong offset!");
static_assert(offsetof(UPyConfig_101641, PhantomBuffID) == 0x00009C, "Member 'UPyConfig_101641::PhantomBuffID' has a wrong offset!");
static_assert(offsetof(UPyConfig_101641, StopCameraInterpTag) == 0x0000A0, "Member 'UPyConfig_101641::StopCameraInterpTag' has a wrong offset!");

// PythonClass PyAbility_101641.PyAbility_101641
// 0x0008 (0x2A40 - 0x2A38)
class UPyAbility_101641 : public UMarvelGameplayAbility
{
public:
	class UAbilityTask_NetworkSyncPoint*          NetSyncTask;                                       // 0x2A38(0x0008)(BlueprintVisible, BlueprintReadOnly, ZeroConstructor, Transient, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

public:
	void BeginPlay();
	bool CanActivate();
	void K2_ActivateAbility();
	void OnBuffRemovedSync();
	void K2_OnEndAbility(bool bWasCancelled);
	void HandleAnimSocketInfo(int32 SocketId, const struct FMarvelAbilitySocketInfo& SocketInfo);
	void NativeOnMontageEvent(const class FString& Tag);
	void DelayEndAbility();

public:
	static class UClass* StaticClass()
	{
		return StaticClassImpl<"PyAbility_101641">();
	}
	static class UPyAbility_101641* GetDefaultObj()
	{
		return GetDefaultObjImpl<UPyAbility_101641>();
	}
};
static_assert(alignof(UPyAbility_101641) == 0x000008, "Wrong alignment on UPyAbility_101641");
static_assert(sizeof(UPyAbility_101641) == 0x002A40, "Wrong size on UPyAbility_101641");
static_assert(offsetof(UPyAbility_101641, NetSyncTask) == 0x002A38, "Member 'UPyAbility_101641::NetSyncTask' has a wrong offset!");

}

